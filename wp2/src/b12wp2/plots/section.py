...
# Created: 2026-09-17
"""Figures of the 2D section analysis, one function per figure.

Reads only the CSVs the polar/mcrit/scorecard stages already wrote (no XFoil
...
scripts/make_plots.py calls these for every airfoil; nothing here decides
where the files go beyond the out_dir it is handed.
"""
...
def _read_polar(stem: str, condition: str) -> pd.DataFrame:
    return pd.read_csv(paths.polar_csv(stem, condition))
...
def plot_cp_distribution(stem: str, color: str, out_dir: Path) -> None:
    cp = pd.read_csv(paths.baseline_cp_csv(stem))
...
def plot_mcrit_sweep(stem: str, color: str, out_dir: Path) -> None:
    sweep = pd.read_csv(paths.mcrit_csv(stem))
    summary = pd.read_csv(paths.MCRIT_SUMMARY_CSV).set_index("airfoil")
...
def plot_criteria_breakdown(colors: dict[str, str], out_dir: Path) -> None:
    detail = pd.read_csv(paths.SCORECARD_DETAIL_CSV, index_col=0)
